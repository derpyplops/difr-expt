...
### Cross-family validation (Phi-4-mini)

The 10-model headline is mostly Qwen/Llama. Cross-family probe on Phi-4-mini
(microsoft/Phi-4-mini-instruct, ~3.8B):

| Config | int student | bf16 sdpa | Δ |
|---|---:|---:|---:|
| matmul-only | 20.5453 | 20.5120 | +0.16% |
| fully-int   | 20.5281 | 20.5120 | +0.08% |

Both within 0.16%. Tested Mistral-7B-Instruct-v0.3-FP8 separately but it uses
the `static` activation scheme which the bundled transformers version doesn't
support; Mistral-Small-3.x-24B variants need >80GB to load 4 model copies for
the comparison (would need a memory-thrifty PPL script to evaluate).

## Per-op contribution to int approximation cost

Cumulative ablation on Qwen2.5-7B with init-from-teacher: add int ops one at a
time and measure PPL. The "+IntSoftmax" entry is the only one that flips
attention into eager mode (since int_softmax replaces eager_attention_forward).

| Configuration | PPL | Δ from previous |
|---|---:|---:|
| baseline (IntLinear + IntEmbedding only, sdpa attn) | 13.0083 | (baseline; matches sdpa 12.95 closely) |
| + IntRMSNorm (sdpa attn) | 13.0077 | -0.005% |
| + IntSiLU (sdpa attn) | 13.0058 | -0.014% |
| + IntSoftmax (forces eager) | 14.2787 | **+9.8%** (mostly sdpa→eager drift) |
| + IntAttnMatmul (eager) | 14.3019 | +0.16% |
| + IntRoPE (eager, full int) | 14.3019 | 0% |

Translation: in the fully-int pipeline, **IntRMSNorm, IntSiLU, IntAttnMatmul,
IntRoPE all cost essentially 0%**. The +9.8% jump at "+IntSoftmax" is the
combined effect of (1) forced switch from sdpa to eager attention (which on
Qwen2.5-7B has its own +8.2% drift unrelated to quantization) and (2) int
approximation in softmax/attn. Bigger softmax LUTs (4k → 32k entries) and
linear interpolation between LUT entries did NOT improve PPL — confirming the
LUT is not the bottleneck. The actual int-softmax cost is ~+1.5% on top of
eager_base, traced to IntLinear ↔ bf16 eager attention numerical interaction
on Qwen2.5-7B's outlier-channel activations. Switching `activation_scheme` to
fp8_e4m3 (wider dynamic range) absorbs that and recovers `Δ vs eager = -2.4%`.
